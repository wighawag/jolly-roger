...
Status: steps zero to three and step five done, step four outstanding. Step zero landed in `ba5a28a`; steps one and two shipped from the `etherplay-connect` monorepo as `@etherplay/delegation` 0.1.0, `@etherplay/connect-core` 0.3.0, `@etherplay/connect` 0.4.0 and `@etherplay/openfort` 0.2.0; step three is this repo's working tree; step five is `main`'s working tree. What remains is step four, `template-commit-reveal` merging down, and the Sepolia redeploys, which are a human decision and have not happened: both branches still point at the pre-delegation implementation they have always shared.

Scope: `jolly-roger` on `variant/full` and on `main`, the `etherplay-connect` monorepo, and `template-commit-reveal` as the first downstream adopter.
...
Done. `contracts/src/core` and `contracts/test/solidity/core` are gone, with them the step-zero signature round-trip test and the JS delegation test, and the `@etherplay/connect-core` devDependency whose only consumer was the latter. `GreetingsRegistry` is `IGreetingsRegistry, Proxied` again, without `setMessageFor`, and the interface no longer has to explain what it deliberately omits. Two things did not match the description above. It was NOT contracts only, and the web side reached three files rather than the one the review of it started from: `web/src/lib/view/index.ts` special-cased `setMessageFor` in two places, choosing the message argument by position and accepting either function name as a pending greeting, which after the contract change was code aimed at a function this branch does not have; a comment in `web/src/lib/context/index.ts` still explained the account parameter as being for an app that sends for its user, which no search for the word would ever have found; and the unit test covering the delegate-sent case was replaced rather than dropped, because deleting it left the rule it existed for (a pending greeting is filed under the authenticated account, never the sending address) asserted nowhere but in prose, with every remaining test sending from the account itself. Its replacement states the same rule without needing delegation to say it, and was checked by keying the implementation off the sender, which it catches and the rest of the suite does not. And the storage-layout test survived rather than being deleted with the rest of the delegation tests: what it guards outlives its original reason, since the contract is live behind an ERC-1967 proxy where a slot shift silently reinterprets every greeting ever stored, so it was rewritten as `test_storageLayoutIsWhatTheProxyExpects` and now pins the first three declarations rather than the absence of a base contract's state.

## Composing with a purchase

Investigated after step three, because a game's onboarding wants one approval for three effects: take the payment, fund the browser's signer, and authorise that signer. The first two compose in one transaction already. The third does not, when the sale is a different contract from the game, because `registerDelegate` takes its authority from `msg.sender` and `msg.sender` at the game is then the sale.

**Decided: the payment belongs on the delegation-carrying contract.** Not a workaround for a limitation, the correct ordering. Only one contract in a call chain is the entry point, and the two candidates have unequal claims on it: a sale is indifferent to who sends (it takes a recipient as a parameter, and reads `msg.sender` only for a free list and an event), while the delegation contract's whole proof is `msg.sender`. Give the slot to the one that needs it. `template-commit-reveal` is already in this shape without having been told to, staking through `addToReserve` on the same proxy that carries `GameDelegation`, so the composition problem does not arise for it at all.

**Nothing in this package changes for any of it.** The signature path reads no `msg.sender` at any depth, so a credential may be submitted through any number of hops by anybody; and where the owner is the sender, the direct path already works. Both were checked rather than assumed.

**Three shapes are refused, and they are one mistake at three scales.** All three take "somebody paid" as evidence of "this is whose account it is", and it never is: **a payment proves that somebody spent money, never whose account they are.** Consent from an account is evidenced by exactly two things, the account is the sender or the account signed. Or, in the form an adopter can act on: **paying for somebody is always safe, speaking for somebody never is** - value flowing to an account needs nobody's permission, because the worst case is a gift, which is exactly why `addToReserve` lets anyone top up anyone; authority over an account needs all of it.

- **Authority asserted in a payload.** The purchase carries the account it is for as data and the contract believes it. Tempting because it costs nothing anywhere and works even for a contract account. It is impersonation: the named account is the one the game then displays and records as having acted, and the defence that the attacker paid for it prices the capability while ignoring the attribution, which has no economic bound. It is also the thing this library's storage layout is keyed owner-first to prevent, in the stronger direction. Found in the field; see `bomber-world/docs/plans/identity-without-consent.md`.
- **A trusted forwarder.** The library permits it with no change, since every `UsingDelegation` function is `virtual` and the library never reads `msg.sender` itself. Refused anyway: it hands one contract, and thereby that contract's upgrade admin, the permanent ability to register any delegate for any owner, including clearing a withdrawal. That is the payload mistake with a contract in the middle, and it breaches the unit of authority by making a second contract able to speak as anybody at the first.
- **A generic register-and-forward.** An entry point that registers from `msg.sender` and then forwards value and arbitrary calldata. An arbitrary-call capability introduced for a specific purpose, landing on a contract that holds state and often custody. Its defensible form is a typed call to a pinned target, which is just the decision above with a hop.

The full evaluation, including wallet batching and deferred registration, the gas and prompt costs of each, and what each does to the value split, is in the ADR on the `work` branch.
...
- The signature path reads no `msg.sender` at any depth: `registerViaSignature` takes `owner` as a parameter and recovers it, `registerDelegateViaSignature` applies no caller check, and the bounds come from `address(this)` and `block.chainid`. Any contract may submit a credential on an owner's behalf, through any number of hops.
- The library never reads `msg.sender` anywhere, and every `UsingDelegation` function is `virtual`. Sender resolution is entirely the adopter's concern, which is what makes a forwarder mechanically possible and is a separate question from whether it is wise.
- A hosted account cannot take the direct route at all, because `createExecutor` reports `cannot-send` without a wallet. Any reasoning about "the owner sends it themselves" excludes that population by construction.
- Registration costs roughly 45k gas by the direct route and 57k by the signature route (the difference being `ecrecover`, the onchain message build and the larger calldata), so `REGISTRATION_GAS = 150_000` covers both and needs no revisiting.
- The origin signer is deterministic, `keccak(sign(originKeyMessage(origin)))`, so the same account at the same origin rediscovers the same signer after a cold start, and with it any funds already sent to that address. Clearing browser storage strands nothing.
