...
**Two repos on a different line had omissions and no machinery, and they have it now.** `ronan-eth` and `conquest-website-2` descend from `template-svelte-tailwind-blog`, not from here, so the machinery was installed at that template - the nearest ancestor they share - and cascaded to both. It reported their deletions on the very cascade that delivered it, and refused to let either merge be committed until each had a reason written down, which is the mechanism doing exactly what it was built for.

**That forced a correction to the rule stated above, and the corrected version is the useful one.** "Do not install it where nothing is omitted" is wrong as stated: `template-svelte-tailwind-blog` omits nothing and is still the right home, because **an ancestor is the distribution point for shared machinery even when it has nothing of its own to guard.** The alternative was the same three files copied into two descendants, which is the divergence-by-copy this tree keeps paying for. What must not be put where it is untrue is a LIST ENTRY, not the machinery. The four nodes measured at zero above still need nothing, for the different reason that they have no descendant lacking it.

**A directory entry is not always the tidier choice, and the blog line is where that became clear.** `conquest-website-2` renamed `routes/blog/` to `routes/devlog/`, so the whole directory is replaced and a directory entry is exactly right. Its two sample posts are the opposite case: `web/src/posts/` is wanted and full of real content, so a directory entry there would assert the directory should be absent and `offshoot-omissions.test.ts` would fail on it immediately. **Name a directory only when the whole directory is what you replaced.** The rename is also the worst shape this conflict takes - git sees a deletion on one side and edits on the other, so every future change the template makes to its blog route stops the cascade on four modify/delete conflicts with the resolution invisible from the diff.
