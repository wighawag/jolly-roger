...
import {
	computeHealth,
	settle,
} from '../../../../src/lib/core/connection/rpcHealth';
...
describe('settle', () => {
	it('is pending while loading (ignores the transient error-cleared blip)', () => {
		expect(settle({loading: true, lastSuccessfulFetch: 1000}).state).toBe(
			'pending',
		);
	...
	it('is ok when settled with a prior successful fetch', () => {
		expect(settle({loading: false, lastSuccessfulFetch: 1000}).state).toBe(
			'ok',
		);
	});
...
describe('health scenarios', () => {
	...
	it('a fresh success clears the banner despite an older error elsewhere', () => {
		const r = computeHealth([
			{
				outcome: {state: 'error', error: {message: 'stale gas error'}},
				at: 1000,
			},
	...
	it('stays unhealthy during an in-flight retry when all are down', () => {
		const loadingBlip: PollingStatus = {
			loading: true,
			error: undefined,
			lastSuccessfulFetch: 500,
		};
